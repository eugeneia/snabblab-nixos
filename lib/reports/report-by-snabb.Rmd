...
### Overall difference between Snabb versions

```{r}
TukeyHSD(aov(score ~ snabb, data=sliced))
```

### Differences between Snabb versions by benchmark.

```{r}
for (b in unique(sliced$benchmark)) {
  cat("TukeyHSD for benchmark ", b, "\n")
  print(TukeyHSD(aov(score ~ snabb, data=filter(sliced, benchmark == b))))
}
```