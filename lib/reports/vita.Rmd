...
loopback = subset(results, subset=(unit == "Gbps" & score))
```
...
p <- p + facet_grid(benchmark ~ pktsize ~ config, scales = "free")
...
cls <- loopback %>%
  group_by(benchmark, snabb, pktsize) %>%
  mutate(group = paste(benchmark, "_", snabb, "_", pktsize, sep=""))
...
p <- p + theme(legend.position="top")
p <- p + facet_grid(benchmark ~ snabb ~ pktsize ~ config, scales = "free")
p <- p + geom_point()
...
if (mult_levels(loopback$snabb))
  TukeyHSD(aov(score ~ snabb, data=loopback))
if (mult_levels(loopback$benchmark))
  TukeyHSD(aov(score ~ benchmark, data=loopback))
if (mult_levels(loopback$config))
   TukeyHSD(aov(score ~ config, data=loopback))
if (mult_levels(loopback$pktsize))
  TukeyHSD(aov(score ~ pktsize, data=loopback))
```
