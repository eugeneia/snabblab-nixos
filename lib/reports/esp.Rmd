...
plot_smooth <- function (df) {
  ...
  p <- p + facet_wrap(~ conf, scales = "free")
...
plot_dist <- function (df) {
  ...
  p <- p + facet_grid(pktsize ~ conf, scales = "free")
...
```{r}
gbps <- subset(esp_tunnel, subset=(unit == "Gbps"))
plot_smooth(gbps)
plot_dist(gbps)
```
...
```{r}
cycles <- subset(esp_tunnel, subset=(unit == "cycles"))
plot_smooth(cycles)
plot_dist(cycles)
```